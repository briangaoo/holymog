...
export async function generateMetadata({
  ...
  return { title: `@${username}'s followers · holymog` };
}
...
export default async function FollowersPage({
  ...
  // Server-side fetch of page 1 — no client-flicker, plus it ships
  // the initial state into FollowList for instant first paint.
  const pool = getPool();
        ...
        <header className="mb-5 flex items-center gap-3">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <span className="text-[14px] text-zinc-400">
              @{profile.data.display_name} · {profile.data.followers_count}
            </span>
          </div>
        </header>
