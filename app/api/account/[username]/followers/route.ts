...
type FollowEntry = {
  ...
  is_viewer: boolean;
  // userStats fields powering smart cosmetics on the row.
  elo: number | null;
...
/**
 * GET /api/account/[username]/followers?page=N
 *
 * Returns up to PAGE_SIZE follower entries for [username], paginated.
 * Each entry carries the basics needed to render an X-style list row:
 * display name, bio preview, avatar, equipped flair/frame for the
 * social pill, followers count of that user, when they followed, plus
 * `viewer_is_following` so the "Follow / Following" button can render
 * server-correct on first paint.
 */
