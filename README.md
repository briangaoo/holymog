...
17. [Privacy](#privacy)
18. [Notes for future contributors](#notes-for-future-contributors)
...
- **Complete** lets the user share to TikTok / Instagram / Snapchat / X / Discord, copy the share image, post to a public leaderboard, retake, or expand a "more detail" panel showing all 30 underlying vision fields plus token-usage / cost breakdown.
...
│   │   ├── leaderboard/route.ts      GET: top scores · POST: insert (with optional photo)
...
│   ├── leaderboard/page.tsx          Public leaderboard list
...
│   ├── LeaderboardModal.tsx          Submit-to-leaderboard form
...
- **`/leaderboard`** — `app/leaderboard/page.tsx`. Fetches `/api/leaderboard` (top 100 by overall, descending), renders rows with rank, avatar (uploaded photo or hashed-name initial circle), name, J/E/S/C sub-scores, tier letter and overall.
...
- `GET` returns `{ entries: LeaderboardRow[] }` (top 100 by `overall` descending). Returns `{ error: 'unconfigured' }` if Supabase env is unset (the page renders a friendly empty state).
- `POST` body: `{ name: string (1..24), scores: FinalScores, imageBase64?: string }`.
  - Rate-limited (`lb:` prefix on the key).
  - Validates scores (`overall` and all four sub-scores must be 0..100 numbers), normalises whitespace.
  - If `imageBase64` is a data-URL with a `image/jpeg|jpg|png` MIME, uploads to the `holymog-faces` Supabase Storage bucket under a UUID path and stores the public URL on the row.
  - Inserts into the `leaderboard` table and returns `{ entry }`.
...
- **`LeaderboardModal`** — Dialog for adding to the public leaderboard. Opt-in photo checkbox shows a 32×32 preview of the captured frame next to the toggle. Submits to `POST /api/leaderboard`, surfaces `rate_limited` / `unconfigured` errors inline.
...
4. **localStorage** holds two keys: `holymog-last-result` (the most recent `{ scores, capturedImage, ts }` for re-hydration) and `holymog-privacy-acknowledged` (a single `'1'` flag so the modal doesn't reappear).
